...
import { NButton, NSpin, NEmpty, NInput, NInputNumber, NSelect, NSwitch, useMessage } from 'naive-ui'
...
// Auxiliary model keys and their labels
const auxiliaryKeys = [
...
// Provider → Model options from appStore.modelGroups
...
// --- Auxiliary: Provider → Model save ---
async function handleAuxProviderChange(taskKey: string, provider: string) {
  const models = getModelsForProvider(provider)
  const model = models[0] || ''
  try {
    // Save provider + model + clear base_url/api_key (Hermes resolves from pool)
...
// --- Fallback: Provider → Model save ---
const fallbackProviderOptions = computed(() => providerOptions.value)
...
// Count configured auxiliary models
