export default {
  ...
  chat: {
    ...
    deleteMessage: '删除消息',
    deleteMessageConfirm: '确定要删除这条消息吗？此操作不可撤销。',
    ...
    segmentUnit: '段',
    messageUnit: '条',
    lineageSegments: '{count} 段关联会话',
    lineageHint: '第 {count} 段更早的记录',
    prevSegment: '上一段',
    nextSegment: '下一段',
    collapse: '收起',
    loadAction: '加载',
    ...
    addWorkspace: '新建工作区',
    addWorkspaceNameHint: '输入名称，例如 我的笔记',
    addWorkspaceRequired: '名称和路径不能为空',
    addWorkspaceFailed: '创建工作区失败',
    browse: '浏览',
    other: '其他',
    runFailed: '运行失败',
    error: '错误',
  ...
  models: {
    ...
    addProvider: '添加模型厂商',
    editProvider: '编辑模型厂商',
    providerManagement: '模型厂商管理',
    providerUpdated: '模型厂商已更新',
    providerAlreadyExists:'模型厂商 "{name}" 已存在',
    providerType: '模型厂商类型',
    preset: '预设',
    custom: '自定义',
    ...
    selectModel: '选择模型...',
    providerAdded: '模型厂商已添加',
    providerDeleted: '模型厂商已删除',
    ...
    local: '本地 ({host})',
    selectProviderRequired:'请选择模型厂商',
  ...
  settings: {
    ...
    agent: {
      ...
      always: '始终',
      never: '从不',
      maxConcurrentChildren: '最大并发子任务',
      maxConcurrentChildrenHint: '同时运行的最大子代理数量',
      maxSpawnDepth: '最大嵌套深度',
      maxSpawnDepthHint: '子代理嵌套调用的最大层数',
      orchestratorEnabled: '启用编排器',
      orchestratorEnabledHint: '允许子代理进一步派生下级子代理',
    },
    memory: {
      basic: '基础',
      advanced: '高级',
      ...
      nudgeInterval: '提醒间隔',
      nudgeIntervalHint: '每隔多少轮对话提醒保存记忆',
      retrievalThreshold: '检索阈值',
      retrievalThresholdHint: '语义搜索的最低相似度阈值',
      maxMemoryEntries: '最大记忆条目',
      maxMemoryEntriesHint: '记忆系统存储的最大条目数',
      flushMinTurns: '刷新最小轮数',
      flushMinTurnsHint: '至少多少轮对话后才刷新记忆到磁盘',
    },
    session: {
      resetPolicy: '重置策略',
      mode: '重置模式',
      ...
      retention: '保留',
      retentionDays: '保留天数',
      retentionDaysHint: '会话数据的保留天数',
      autoPrune: '自动清理',
      autoPruneHint: '自动删除超过保留期限的会话',
      vacuumAfterPrune: '清理后压缩',
      vacuumAfterPruneHint: '删除会话后执行数据库压缩',
    },
    workspace: {
      empty: '暂无工作区，添加一个开始吧。',
      add: '添加工作区',
    ...
    auxiliary: {
      ...
      maxConcurrency: '最大并发',
      maxConcurrencyHint: '同时进行的最大搜索数量',
      downloadTimeout: '下载超时',
      downloadTimeoutHint: '图片下载超时时间（秒）',
      apiMode: 'API 模式',
      apiModeHint: '使用 Chat Completions 或 Responses API',
    ...
    approvals: {
      ...
      autoOff: '自动关闭',
      cronMode: '定时任务审批',
      cronModeHint: '定时任务的审批方式',
      deny: '拒绝',
      allow: '允许',
      timeout: '超时时间',
      timeoutHint: '等待审批的超时时间（秒）',
    },
    humanDelay: {
      humanDelay: '模拟延迟',
      mode: '模式',
    ...
    security: {
      securityBlock: '安全',
      redactSecrets: '脱敏密钥',
      redactSecretsHint: '自动隐藏输出中的 API Key 等密钥信息',
      allowPrivateUrls: '允许内网地址',
      allowPrivateUrlsHint: '允许访问 localhost 和内网 URL',
    },
  },
  ...
  changelog: {
    ...
    new_0_5_1_7: '优化模型厂商 卡片中模型列表布局（固定高度、标签对齐）',
    ...
    new_0_4_7_5: '新增阿里云编码计划内置模型厂商，支持 .env base_url 覆盖',
    new_0_4_7_6: '启动时跳过远程配置文件以防止卡住',
    new_0_4_7_7: '检测并展示被静默吞没的运行错误',
    new_0_4_7_8: '模型厂商 感知的上下文长度查询',
    new_0_4_7_9: '切换时重置 config.model 并解析 CLI 自定义模型厂商',
    new_0_4_7_10: '删除内置模型厂商 时清除 .env 中的 base_url_env',
    ...
    new_0_4_4_4: '新增 StepFun 和 Nous Portal模型厂商 支持',
    ...
    new_0_4_2_5: '修复内置模型厂商 检测和模型匹配问题',
  },
  ...
  slashCmd: {
    ...
    desc: {
      ...
      stop: '终止所有后台进程',
      background: '后台运行提示词',
      agents: '查看活跃的 Agent 和运行中的任务',
      queue: '将提示词排队到下一轮',
      steer: '在下一次工具调用后注入消息',
      status: '显示会话信息',
      profile: '显示当前 Profile 和主目录',
      resume: '恢复之前命名的会话',
      model: '切换当前会话的模型',
      personality: '设置预设性格',
      yolo: '切换 YOLO 模式（跳过所有危险命令审批）',
      reasoning: '管理推理努力程度和显示',
      fast: '切换快速模式',
      voice: '切换语音模式',
      reloadMcp: '从配置重新加载 MCP 服务器',
      commands: '浏览所有命令和技能',
      help: '显示可用命令',
      usage: '显示当前会话的 token 用量',
      insights: '显示用量分析',
      debug: '上传调试报告并获取分享链接',
      approve: '批准一个待审批的危险命令',
      deny: '拒绝一个待审批的危险命令',
      sethome: '设置当前聊天为主频道',
